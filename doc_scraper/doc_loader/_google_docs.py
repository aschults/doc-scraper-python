...
class DocDownloader():
    ...
    def __init__(self,
                 creds: Optional[credentials.Credentials] = None) -> None:
        """Create an instance.

        Args:
            creds: Credentials to use when accessing the doc.
        """
        self._creds = creds or _auth.get_credentials_from_files()
    ...
    def _get_raw_html(self, doc_id: str) -> str:
        ...
        # pylint: disable=no-member
        logging.info('Fetching from Google Drive: %s, client id: %s', doc_id,
